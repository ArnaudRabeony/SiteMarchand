<div id="welcomeContainer">
	<div class="row" id="bannerContainer">
		<div class="col-md-10"></div>
	</div>
	<div class="row imagesContainer">
		<div class="col-md-6">
			<div class="curtain">
				<img class="img-responsive" src="images/running.jpg" alt="running">
				<a href="#"><p>RUNNING <br><i class="fa fa-2x fa-th"></i></p></a>
			</div>
		</div>
		<div class="col-md-6">
			<div class="curtain">
				<img class="img-responsive" src="images/winterSports.jpg" alt="winter">
				<a href="#"><p>SPORTS D'HIVER <br><i class="fa fa-2x fa-th"></i></p></a>
			</div>
		</div>
	</div>
	<div class="row imagesContainer">
		<div class="col-md-6">
			<div class="curtain">
				<img class="img-responsive" src="images/summerSports.jpg" alt="summer">
				<a href="#"><p>SPORTS D'ÉTÉ <br><i class="fa fa-2x fa-th"></i></p></a>
			</div>
		</div>
		<div class="col-md-6">
			<div class="curtain">
				<img class="img-responsive" src="images/futsal.jpg" alt="futsal">
				<a href="#"><p>SPORTS D'INTERIEUR <br><i class="fa fa-2x fa-th"></i></p></a>
			</div>
		</div>
	</div>
</div>